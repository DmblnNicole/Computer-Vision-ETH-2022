...
def main():
  ...
  # Show the images
  #PlotImages(images)
  plt.imread("/home/nicole/ETH/Computer_Vision/Labs/Lab07/Assignment_07/data/images/0000.png")
  plt.show()
  ...
  # TODO
  # Set pose of image 1 to identity
  R0 = np.array([[1,0,0],[0,1,0],[0,0,1]])
  t0 = np.array([0,0,0])
  e_im1.SetPose(R0, t0)
  # Get four possible transformations
  poses = DecomposeEssentialMatrix(E)
  # Triangulate points with four possible poses
  for i in range(4):
    if i == 0:
      e_im2.SetPose(poses[i][0], poses[i][1])
      points3D, im1_corrs, im2_corrs = TriangulatePoints(K, e_im1, e_im2, e_matches)
      #print("num points that lie in front of both cameras: ", len(points3D))
    if i == 1:
      e_im2.SetPose(poses[i][0], poses[i][1])
      points3D, im1_corrs, im2_corrs = TriangulatePoints(K, e_im1, e_im2, e_matches)
      #print("num points that lie in front of both cameras: ", len(points3D))
    ...
    if i == 3:
      ...
      points3D, im1_corrs, im2_corrs = TriangulatePoints(K, e_im1, e_im2, e_matches)
      #print("num points that lie in front of both cameras: ", len(points3D))
  # pose i==2 is the best pose, that is the most points lie in front of both cameras
  ...
  e_im1.SetPose(R0, t0)
  # best pose
  e_im2.SetPose(poses[2][0], poses[2][1])
